...
export function getNonEmptyQuestions(questions: Question[]): Question[] {
    return questions.filter(
        (question) =>
            question.body !== "" ||
            question.expected !== "" ||
            question.options.length > 0 // Makes sure question is not considered by any means to be empty
    );
}
...
export function sumPoints(questions: Question[]): number {
    return questions.reduce(
        (pointTotal, question) => pointTotal + question.points,
        0
    );
}
...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce((total, question) => {
        return question.published ? total + question.points : total; // Adds if question is published
    }, 0);
}
...
export function toCSV(questions: Question[]): string {
    ...
    return [headers].concat(data).join("\n");
}
...
export function sameType(questions: Question[]): boolean {
    if (questions.length === 0) return true; // Empty array
    const firstType = questions[0].type; // Establishes type
